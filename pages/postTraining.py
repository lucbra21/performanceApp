...
# ============================================================================
    ...
    def update_nuevos_graficos(selected_date, selected_statistic):
        ...
        try:
            ...
            COLOR_BARRAS = "rgba(200, 216, 242, 0.6)"      # Azul claro suave
            COLOR_BARRAS_BORDER = "#4a90e2"                 # Azul oscuro para borde
            COLOR_CIRCULOS = "#0052cc"                      # Azul profundo
            COLOR_LINEA = "#e74c3c"                         # Rojo vibrante
            COLOR_TEXTO = "#2c3e50"                         # Gris oscuro
            COLOR_GRID = "rgba(200, 200, 200, 0.2)"        # Grid suave

            # --- Graficar ---
            fig_hsr = go.Figure()

            # 1. BARRAS (Abs HSR) - Al fondo, eje derecho (y2)
            fig_hsr.add_trace(go.Bar(
                ...
                marker=dict(
                    color=COLOR_BARRAS,
                    line=dict(color=COLOR_BARRAS_BORDER, width=1.5)
                ),
                hovertemplate='<b>%{x}</b><br>Abs HSR: %{y:.0f} m<extra></extra>',
                opacity=0.7
            ))

            # 2. LÍNEA ROJA (% cumplimiento) - En el medio
            fig_hsr.add_trace(go.Scatter(
                x=players_x,
                y=y_line_hsr,
                mode='lines+markers',
                name=f"Cumplimiento 94min ({selected_statistic})",
            ...
            # 3. CÍRCULOS AZULES (HSR/min) - Al frente, eje izquierdo (y)
            fig_hsr.add_trace(go.Scatter(
                x=players_x,
                y=hsr_min_values,
                mode='markers+text',
                ...
                hovertemplate='<b>%{x}</b><br>HSR/min: %{y:.2f} m/min<extra></extra>'
            ))

            # Agregar labels de porcentaje por encima de la línea roja
            fig_hsr.add_trace(go.Scatter(
                x=players_x,
                y=y_line_hsr,
                mode='text',
                ...
                text=text_line_hsr,
                textposition='top center',
                textfont=dict(color=COLOR_LINEA, size=12, family='Arial, sans-serif'),
                hoverinfo='skip',
                showlegend=False
            ))

            # --- Layout con estilos mejorados ---
            fig_hsr.update_layout(
                title=dict(
                    text='<b>HSR: Absoluto, Cumplimiento 94min y Intensidad por Jugador</b>',
                    x=0.5,
                    xanchor='center',
                    font=dict(size=18, color=COLOR_TEXTO, family='Arial, sans-serif')
                ),
                xaxis=dict(
                    title='<b>Jugador</b>',
                    titlefont=dict(size=14, color=COLOR_TEXTO),
                    showgrid=True,
                    gridwidth=1,
                ...
                yaxis=dict(
                    title='<b>HSR/min (m/min) y % Cumplimiento</b>',
                    titlefont=dict(size=13, color=COLOR_TEXTO),
                    showgrid=True,
                    gridwidth=1,
                    gridcolor=COLOR_GRID,
                    zeroline=False,
                    tickfont=dict(size=11, color=COLOR_TEXTO),
                    range=[0, max(hsr_min_values) * 1.2] if len(hsr_min_values) > 0 else [0, 10]
                ),
                yaxis2=dict(
                    title='<b>Abs HSR (m)</b>',
                    titlefont=dict(size=13, color=COLOR_TEXTO),
                    ...
                    zeroline=False,
                    tickfont=dict(size=11, color=COLOR_TEXTO),
                    range=[0, df_hsr_sorted["Abs HSR(m)"].max() * 1.15] if len(df_hsr_sorted) > 0 else [0, 100]
                ),
                plot_bgcolor='rgba(248, 249, 250, 0.5)',
                paper_bgcolor='white',
                font=dict(family='Arial, sans-serif', size=12, color=COLOR_TEXTO),
                height=700,
                margin=dict(t=100, b=80, l=70, r=70),
                hovermode='x unified',
                legend=dict(
                    x=0.01,
                    y=0.99,
                    bgcolor='rgba(255, 255, 255, 0.9)',
                    bordercolor=COLOR_BARRAS_BORDER,
                    borderwidth=1,
                    font=dict(size=11),
                    yanchor='top',
                    xanchor='left'
                ),
                # Mejorar separación entre trazas
                showlegend=True,
                dragmode='zoom'
            )
